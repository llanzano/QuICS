...
% v2 added option for selecting radially averaged ('r') or 'x' or 'y' ACF
% fixed several bugs 



function ResultsICS=QuICS_v2(filename );

% default values
px=0.044; %px size in um
mode='a';
maxlagd=20;
modeav='r'; % radial average  (r or x or y)
Sat1=0; % for single image analysis
Thr1=0; % for single image analysis
%open files ... 
...
% downsampling from checkerboard (if odd will cut one row-col)
X1=floor(X/2);
Y1=floor(Y/2);
for i=1:X1
    for j=1:Y1
...
[Mask_do]=uint16(simple_oversample_image(Mask1, 1.9999));
if ~iseven(X)
xch1=xch1(1:end-1,:);   
Mask=Mask(1:end-1,:);
end
...
CCF=simple_ICCS_CCFmean_mode(x1pad_e,x1pad_o, modeav)*m*n./(m*n-Np_e);
% CCFref=simple_ICCS_CCFmean(x1pad_e,x1pad_e)*m*n./(m*n-Np_e);

%calculate standard auto corr function
[m,n]=size(x1pad);
ACF1=simple_ICCS_CCFmean_mode(x1pad,x1pad, modeav)*m*n./(m*n-Np);  % (m*n - Np) is the # pixels of the ROI=area ROI
...
prompt2 = {'Min lag:','Points for fit:','Pixel size (um)','Fit auto (a) or cross (c)', 'Radial average (r) or x or y'}; 
dlg_title2 = 'Set paramaters for fit'; 
num_lines = 1;
def2 = {num2str(lag0),num2str(lagfit),num2str(px),mode, modeav}; 
...
for i=1:NumberOfAngles
   ...
   profile=y((rt-1).*y1y+ct);   % elements are selected using linear indexing. first point has noise (only in ACF2) 
...
function  Output=simple_ICCS_CCFmean_mode(x1,x2, modeav)
...
if iseven(R)
ACF1=flipud(ACF(2:r0,c0:end));  % quadrant top right only including g(0,0) =the central ACF point 
else
ACF1=flipud(ACF(1:r0,c0:end));  % quadrant top right only including g(0,0) 
end
ACF2=ACF(r0:end,c0:end);        % quadrant bottom right only including g(0,0)

ACFy=[ACF2(1,1) (0.5*(ACF1(2:end,1) + ACF2(2:end,1) ))' ] ;  %av in y
ACFx=[ACF2(1,1) (0.5*(ACF1(1,2:end) + ACF2(1,2:end) )) ] ;   %av in  x
